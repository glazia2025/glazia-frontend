...
export default function CreateQuotationPage() {
  ...
  const getNextQuotationNumber = () => {
    ...
    const lastQuotationNumber = existingQuotations.length > 0
      ? Math.max(...existingQuotations.map((q: { quotationNumber: string }) => {
          const match = q.quotationNumber.match(/QUO-(\d+)/);
          return match ? parseInt(match[1]) : 0;
        }))
      : 0;
    return `QUO-${String(lastQuotationNumber + 1).padStart(3, '0')}`;
  };
  ...
  // Set quotation number on component mount
  useEffect(() => {
  ...
  function getColorFinishRate(colorFinish: string): number {
    console.log('color finish', colorFinish, colorFinish === "Grey-Tex 2200");
  if (
...
function getTotalHandleCost(item: QuotationItem) {
  const count =
    handleCountTable[item.systemType]?.[item.series]?.[item.description] ?? 0;

  const rate =
    handleRateTable[item.systemType]?.[item.handleType]?.[item.handleColor] ?? 0;
  return (count * rate)/item.area;
}

function getMeshRate(item: QuotationItem) {
  const rate =
    meshRateTable[item.meshPresent]?.[item.meshType] ?? 0;
    
    console.log(rate, item.meshType, item.meshPresent, 'mesh');
  return rate;
}
  ...
  const handleSave = () => {
    // Ensure we have the latest quotation number before saving
  ...
  const handleDownloadPDF = async () => {
    // Ensure we have the latest quotation number before generating PDF
    const finalQuotationNumber = quotationDetails.quotationNumber || getNextQuotationNumber();
