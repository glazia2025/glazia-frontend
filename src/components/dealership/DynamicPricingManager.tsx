...
export default function DynamicPricingManager({ fabricators, request, onMessage, onError }: {
  ...
  const save = async () => {
    ...
    try {
      const data = await request(`/api/dealership/fabricators/${selectedId}/dynamic-pricing`, { method: 'PUT', body: JSON.stringify(pricing) });
  ...
  const pricingTable = (title: string, section: keyof Pricing) => {
    ...
    return <section className="overflow-hidden rounded-xl border border-gray-200 bg-white">
      ...
      <div className="max-h-[460px] overflow-auto"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-white text-xs uppercase tracking-wide text-gray-500"><tr><th className="px-5 py-3">Category / item</th><th className="w-44 px-5 py-3">Pricing (%)</th></tr></thead><tbody className="divide-y divide-gray-100">{rows.map(([key, value]) => <tr key={key}><td className="px-5 py-3 font-medium text-gray-800">{key}</td><td className="px-5 py-2"><div className="flex items-center rounded-lg border border-gray-300 bg-white focus-within:border-[#124657]"><input type="number" step="0.01" value={value} onChange={(event) => updateRate(section, key, event.target.value)} className="min-w-0 flex-1 rounded-lg px-3 py-2 outline-none"/><span className="pr-3 text-gray-400">%</span></div></td></tr>)}{!rows.length && <tr><td colSpan={2} className="px-5 py-10 text-center text-gray-500">No matching pricing rules.</td></tr>}</tbody></table></div>
    </section>;
  };
  ...
  return <div className="grid gap-6 xl:grid-cols-[300px_1fr]">
    ...
    <div>{selected && <div className="mb-5 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div><p className="text-xs font-semibold uppercase tracking-wide text-[#124657]">Dynamic pricing for</p><h2 className="mt-1 text-xl font-bold text-gray-900">{selected.name}</h2><p className="text-sm text-gray-500">{selected.email} · {selected.phoneNumber}</p></div><button onClick={save} disabled={saving || loading} className="inline-flex items-center gap-2 rounded-lg bg-[#124657] px-5 py-2.5 font-medium text-white disabled:opacity-50"><Save size={17}/>{saving ? 'Saving…' : 'Save pricing'}</button></div>}
      <div className="mb-4 flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 shadow-sm"><Search size={17} className="text-gray-400"/><input value={rateSearch} onChange={(event) => setRateSearch(event.target.value)} placeholder="Search hardware, profile category or size" className="min-w-0 flex-1 py-3 text-sm outline-none"/></div>
      {loading ? <div className="rounded-xl border bg-white p-10 text-center text-gray-500">Loading pricing…</div> : <div className="grid gap-5 2xl:grid-cols-2">{pricingTable('Hardware Dynamic Pricing', 'hardware')}{pricingTable('Profile Dynamic Pricing', 'profiles')}</div>}
      <p className="mt-4 text-xs text-gray-500">A value of 100 uses the standard price. These rates apply only to the selected fabricator.</p>
    </div>
  </div>;
}
