...
import type { PaymentQuote } from './PaysharpCheckout';
...
const LegacyOrderPlacement: React.FC<OrderPlacementProps> = ({ onOrderSuccess, onCancel,items,sourceOrderId,paymentQuote,checkoutKey,onResumePaysharp,}) => {
...
const finalTotal = paymentQuote.totalPaise / 100;
  ...
  ...(sourceOrderId ? { sourceOrderId } : {
    products: orderItems.map(item => ({ productId: String(item.id), quantity: item.quantity }))
  }),
  payment: { amount: finalTotal, proof: paymentProof }
  ...
  const renderProcessingStep = () => (
    <div className="text-center space-y-6">
      <div className="flex justify-center">
        <Loader2 className="w-16 h-16 animate-spin text-[#EE1C25}" />
      </div>
      <div>
...
export default LegacyOrderPlacement;
